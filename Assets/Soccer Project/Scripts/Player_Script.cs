...
public class Player_Script : MonoBehaviour {
	...
	void  Update() {
		...
		case Player_State.OPPONENT_ATTACK://klo musuh yg lg attack
		...
		case Player_State.SHOOTING:
			...
			if (animation["shoot"].normalizedTime > 0.2f && sphere.owner == this.gameObject) {//wkt animasi ud 0.2 bagian, bola ud lepas dr player,ownerny jd kosong
				...
				if ( gameObject.tag == "PlayerTeam1" ) {
					...
					sphere.gameObject.GetComponent<Rigidbody>().velocity = new Vector3(0, sphere.timeShootButtonPressed * 30.0f, transform.forward.z*30.0f*Strong);//TODO bisa x dirandom sama z ksh pengaruh dr power pemain
...
//					}
		...
		case Player_State.STOLE_BALL_NO_CHECK:
			...
			if(!sphere.owner || sphere.owner.tag != gameObject.tag){//tdnya ga pake ginian if tp jd ngebug kiperny ttp ngejjar walopun ud diambel tmnnya
		...
		if ( coll.collider.transform.gameObject.tag == "Ball" && !gameObject.GetComponent<Player_Script>().temporallyUnselectable ) {
			...
			if ( relativePos.y < 0.35f ) { //bola jd milik player ini klo bolanya ada di darat bkn di udara
				...
				if ( gameObject.tag == "OponentTeam" ) {//klo ini enemy,suruh nyerang enemyny
					state = Player_Script.Player_State.OPPONENT_ATTACK;
				}
