...
public class Joystick_Script : MonoBehaviour
{
    ...
    */
    ...
    private void Awake()
    {
        ...
        if (gui.texture == null)//cek apakah GUI texture aktif,klo aktif pake joystik,gui texture ada dimasing" button
        {
        ...
        else
        {
            ...
            guiTouchOffset.y = defaultRect.height * 0.5f;//set jarak antar button

            /* Cache the center of the GUI, since it doesn't change */
            guiCenter.x = defaultRect.x + guiTouchOffset.x; //set titik tengah button
            guiCenter.y = defaultRect.y + guiTouchOffset.y;

            /* Let's build the GUI boundary, so we can clamp joystick movement */
            guiBoundary.min.x = defaultRect.x - guiTouchOffset.x;//bikin boundary jd kaga nempel antar button
    ...
    public void Restart()
    {
        ...
        fingerDownPos = Vector2.zero;//reset posisi joystik ketengah

        if (touchPad && fadeGUI)
        {
            gui.color = new Color(gui.color.r, gui.color.g, gui.color.b, 0.025f);//set color joystik,klo fade gui nyala jd alphanya cmn 1/4
        }
    }
    ...
    private void Update()
    {
        ...
        if (tapTimeWindow > 0)//klo waktu antara tap blm abis krgin terus
        {
        ...
        else
        {
            ...
            {
                Touch touch = Input.GetTouch(i);//ambil posisi touch per finger
        ...
        if (!touchPad)
        {
            ...
            position = new Vector2//ambil posisi joystik skrg
                (
        ...
        if (absoluteX < deadZone.x)
        {
            ...
            position = new Vector2(0, position.y);//klo joystikny kcl bgt geraknya bikin 0 aja
        }
        else if (normalize)//buat normalisasi posisi
        {
            /* Rescale the output after taking the dead zone into account */
            position = new Vector2(Mathf.Sign(position.x) * (absoluteX - deadZone.x) / (1 - deadZone.x), position.y);//mathf.sign itu return 1 klo + ato 0 angkanya, -1 klo minus
        }
